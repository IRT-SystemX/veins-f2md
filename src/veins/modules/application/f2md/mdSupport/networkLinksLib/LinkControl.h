...
public:
     ...
     */
    double calculateDistance(const veins::Coord& pos, double deltaX, double deltaY);

protected:
    F2MDParameters * params;
    struct CacheKey {
        const veins::Coord senderPos;
        const veins::Coord receiverPos;

        CacheKey(const veins::Coord& senderPos, const veins::Coord& receiverPos) :
