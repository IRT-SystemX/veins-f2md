...
Link::Link(const Link& link){
    for (const veins::Coord& cd : link.coords) {
        coords.push_back(cd);
    }
    bboxP1 = link.bboxP1;
    bboxP2 = link.bboxP2;
...
void Link::setShape(Coords shape) {
	coords = shape;
	bboxP1 = veins::Coord(1e7, 1e7);
	bboxP2 = veins::Coord(-1e7, -1e7);
...
const veins::Coord Link::getBboxP1() const {
	return bboxP1;
}

const veins::Coord Link::getBboxP2() const {
	return bboxP2;
}
...
double Link::getDistance(const veins::Coord * pos) const{
    double dist = 10000;
    const veins::Coord* oldCd = NULL;
    bool firstelem = true;

    double localdist = 0;
   for (const veins::Coord& cd : coords) {
       if(firstelem){
           oldCd = &cd;
           firstelem = false;
       }else{
